...
public class CaculateQuyuSaleInfoByType_P2_ForZixie implements IBasicServiceInterface {

    private static final Logger log = LoggerFactory.getLogger(CaculateQuyuSaleInfoByType_P2_ForZixie.class);
    ...
    public void generateText(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        for( QuyuBasicInfo quyuBasicInfo : BasicCommonValues.quYuBasicInfoListWithOrder){
            ...
            Map<String, Integer> cityNumberMap =  BasicCommonValues.quyuCityNumberPerPriority.get(quyuName).get(BasicConstants.TYPE_ZIXIE);
        ...
        String fileName = BasicConfiguredValues.PAGE2And3_OUTPUT_DIRECTORY +  "自卸车城市覆盖程度.txt";
        OutputToTxtUtil.writeTo( outputStringBuffer, fileName);
        log.info("Write the      自卸车覆盖度 to file: " + fileName);
...
//        OutputToTxtUtil.writeTo(outputStringBuffer, "");
    ...
    private Map<String, Map<Integer, Map<String, Integer>>>  readByNetwork(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        Map<String, Map<Integer, Map<String, Set<String>>>> existingCityOfZixie = new HashMap<>();

//        第一遍是为了读取： 销量 跟城市列表 / city and year
        for (JinpinOneLinePOJO oneLine : jinpinOneList) {
            Map<Integer, Map<String, Set<String>>> citySetPerYearPriority = existingCityOfZixie.get(oneLine.quYu);
            if(citySetPerYearPriority==null){
                citySetPerYearPriority = new HashMap<>();
                existingCityOfZixie.put(oneLine.quYu, citySetPerYearPriority);
            }
            ...
            Set<String> citySet = citySetPerPriority.get( oneLine.zixieTypePriority);
            if( citySet==null){
                citySet = new HashSet<>();
                citySetPerPriority.put(oneLine.zixieTypePriority, citySet );
            }
            citySet.add(oneLine.city);
        }

        Map<String, Map<Integer, Map<String, Integer>>> output_cityNumberOfZixie = new HashMap<>();
        for( Map.Entry<String, Map<Integer, Map<String, Set<String>>>> entry : existingCityOfZixie.entrySet() ){
            ...
            output_cityNumberOfZixie.put(quyu, output_cityNumberPerPriorityYear);
        }
        return output_cityNumberOfZixie;
    }
