 ...
 */
@Component
public class CaculateSellerInfoByType_P14 implements IBasicServiceInterface {
    ...
    private void readJinpinList( List<JinpinOneLinePOJO> jinpinOneList){
        ...
        //key: quyu->year->specialPriority : value: seller Number.
        Map<String, Map<Integer, Map<String, Integer>>> quyuAllSellerNumberPer = new HashMap<>();
        //key: quyu->year->specialPriority->globalPriority: value: seller Number.
        Map<String, Map<Integer, Map<String, Map<String,Integer>>>> quyuPrioritySellerNumberPerYear = new HashMap<>();

        //All sellerNumber Per year.
        Map<String, Map<Integer, Integer>> quyuAllSellerNumberPerYear = new HashMap<>();
        //All special seller number per year. seems useless.
        ...
        for(SellSpecialInfo sellSpecialInfo : specialSellersSet  ){
            ...
            //for output
            Map<Integer, Map<String, Map<String,Integer>>> specialSellerNumberPerPriorityYear = quyuPrioritySellerNumberPerYear.get(quYu);
            if( specialSellerNumberPerPriorityYear ==null ){
                specialSellerNumberPerPriorityYear = new HashMap<>();
                quyuPrioritySellerNumberPerYear.put(quYu, specialSellerNumberPerPriorityYear);
            }
            ...
            for( int year=2012; year<=2018; year++ ){
                ...
                //prepare for output
                //This map: key is specialSellerType->globalType, value is Number.
                Map<String, Map<String,Integer>> specialSellerNumberPerPriority = specialSellerNumberPerPriorityYear.get(year);
                if( specialSellerNumberPerPriority==null){
                    specialSellerNumberPerPriority = new HashMap<>();
                    specialSellerNumberPerPriorityYear.put(year, specialSellerNumberPerPriority);
                }
                ...
                if( !level.equals(BasicConstants.SPECIAL_SELL_TYPE_Empty)){
                    Map<String,Integer> sellerNumberPerGlobalPriority = specialSellerNumberPerPriority.get(  level );
                    if( sellerNumberPerGlobalPriority==null ){
                        sellerNumberPerGlobalPriority = new HashMap<>();
                        specialSellerNumberPerPriority.put(  level, sellerNumberPerGlobalPriority );
                    }
                    Integer sumSellerNumberPerSellerSpecialPerGlobalPriority = sellerNumberPerGlobalPriority.get(sellSpecialInfo.globalPriorityPerYearMap.get(year) );
                    if( sumSellerNumberPerSellerSpecialPerGlobalPriority== null){
                        sellerNumberPerGlobalPriority.put(sellSpecialInfo.globalPriorityPerYearMap.get(year), 1 );
                    }else{
                        sellerNumberPerGlobalPriority.put(sellSpecialInfo.globalPriorityPerYearMap.get(year), sumSellerNumberPerSellerSpecialPerGlobalPriority+1 );
                    }
                    // 非形象店
        ...
        for( QuyuBasicInfo quyuBasicInfo: BasicCommonValues.quYuBasicInfoListWithOrder){
            ...
            Map<Integer, Map<String, Map<String,Integer>>> sepcialSellerNumberPerGlobalPriority = quyuPrioritySellerNumberPerYear.get(quYuName);
            ...
            if(sepcialSellerNumberPerGlobalPriority==null){
                sepcialSellerNumberPerGlobalPriority = new HashMap<>();
            }
            ...
            for( int i=2012;i<=2018; i++){
                ...
                Map<String, Map<String,Integer>> specailSellerNumberPerPriority =   sepcialSellerNumberPerGlobalPriority.get(i);
                if(specailSellerNumberPerPriority==null){
                    specailSellerNumberPerPriority = new HashMap<>();
                }
...
//                }
                for( String specialSellerLevel : outputTitleSequence ){
                    //key: quyu->year->specialPriority : value: seller Number.
//                    Map<String, Map<Integer, Map<String, Integer>>> quyuAllSellerNumberPer = new HashMap<>();
                    //key: quyu->year->specialPriority->globalPriority: value: seller Number.
//                    Map<String, Map<Integer, Map<String, Map<String,Integer>>>> quyuPrioritySellerNumberPerYear = new HashMap<>();

                    Map<String,Integer> sellerNumberPerGlobalPriority = specailSellerNumberPerPriority.get(specialSellerLevel);
                    if( sellerNumberPerGlobalPriority==null){
                        sellerNumberPerGlobalPriority = new HashMap<>();
                    }

                    int totN = 0;
                    for( String globalPriority : outputTitleSequence ){
                        int outputSellerNumber = sellerNumberPerGlobalPriority.get(globalPriority)==null?0:sellerNumberPerGlobalPriority.get(globalPriority);
                        sb.append(outputSellerNumber).append(BasicConstants.TAB_WORD_BREAK);
                        totN += outputSellerNumber;
                    }
                    sb.append(totN).append(BasicConstants.TAB_WORD_BREAK);
                    double totalSpecialNumber = (double) totN;
...
//            sb.append(BasicConstants.ENTER_LINE_BREAK);
        ...
        OutputToTxtUtil.writeTo( outputLines, BasicConfiguredValues.PAGE14_OUTPUT_DIRECTORY+"各区域形象店分布情况.txt" );
    }
