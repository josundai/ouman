...
public class CaculateQuyuSaleInfoByType_P2_ForPinban implements IBasicServiceInterface {

    private static final Logger log = LoggerFactory.getLogger(CaculateQuyuSaleInfoByType_P2_ForPinban.class);
    ...
    public void generateText(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        for( QuyuBasicInfo quyuBasicInfo : BasicCommonValues.quYuBasicInfoListWithOrder){
            ...
            Map<String, Integer> cityNumberMap =  BasicCommonValues.quyuCityNumberPerPriority.get(quyuName).get(BasicConstants.TYPE_PINBAN);
        ...
        String fileName = BasicConfiguredValues.PAGE2And3_OUTPUT_DIRECTORY +  "平板车城市覆盖程度.txt";
        OutputToTxtUtil.writeTo( outputStringBuffer, fileName);
        log.info("Write the       平板车覆盖度 to file: " + fileName);
...
//        OutputToTxtUtil.writeTo(outputStringBuffer, "");
    ...
    private Map<String, Map<Integer, Map<String, Integer>>>  readByNetwork(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        Map<String, Map<Integer, Map<String, Set<String>>>> existingCityOfPinban = new HashMap<>();

//        第一遍是为了读取： 销量 跟城市列表 / city and year
        for (JinpinOneLinePOJO oneLine : jinpinOneList) {
            Map<Integer, Map<String, Set<String>>> citySetPerYearPriority = existingCityOfPinban.get(oneLine.quYu);
            if(citySetPerYearPriority==null){
                citySetPerYearPriority = new HashMap<>();
                existingCityOfPinban.put(oneLine.quYu, citySetPerYearPriority);
            }
            ...
            Set<String> citySet = citySetPerPriority.get( oneLine.pinbanTypePriority);
            if( citySet==null){
                citySet = new HashSet<>();
                citySetPerPriority.put(oneLine.pinbanTypePriority, citySet );
            }
            citySet.add(oneLine.city);
        }

        Map<String, Map<Integer, Map<String, Integer>>> output_cityNumberOfPinban = new HashMap<>();
        for( Map.Entry<String, Map<Integer, Map<String, Set<String>>>> entry : existingCityOfPinban.entrySet() ){
            ...
            output_cityNumberOfPinban.put(quyu, output_cityNumberPerPriorityYear);
        }
        return output_cityNumberOfPinban;
    }
