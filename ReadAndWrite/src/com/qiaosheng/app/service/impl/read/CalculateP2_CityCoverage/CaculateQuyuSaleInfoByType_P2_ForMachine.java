...
public class CaculateQuyuSaleInfoByType_P2_ForMachine implements IBasicServiceInterface {

    private static final Logger log = LoggerFactory.getLogger(CaculateQuyuSaleInfoByType_P2_ForMachine.class);
    ...
    public void generateText(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        for( QuyuBasicInfo quyuBasicInfo : BasicCommonValues.quYuBasicInfoListWithOrder){
            ...
            Map<String, Integer> cityNumberMap =  BasicCommonValues.quyuCityNumberPerPriority.get(quyuName).get(BasicConstants.NETWORKTYPE_MACHINE);
        ...
        String fileName = BasicConfiguredValues.PAGE2And3_OUTPUT_DIRECTORY +  "工程车城市覆盖程度.txt";
        OutputToTxtUtil.writeTo( outputStringBuffer, fileName);
        log.info("Write the      工程车覆盖度 to file: " + fileName);
...
//        OutputToTxtUtil.writeTo(outputStringBuffer, "");
    ...
    private Map<String, Map<Integer, Map<String, Integer>>>  readByNetwork(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        Map<String, Map<Integer, Map<String, Set<String>>>> existingCityOfMachine = new HashMap<>();

//        第一遍是为了读取： 销量 跟城市列表 / city and year
        for (JinpinOneLinePOJO oneLine : jinpinOneList) {
            Map<Integer, Map<String, Set<String>>> citySetPerYearPriority = existingCityOfMachine.get(oneLine.quYu);
            if(citySetPerYearPriority==null){
                citySetPerYearPriority = new HashMap<>();
                existingCityOfMachine.put(oneLine.quYu, citySetPerYearPriority);
            }
            ...
            Set<String> citySet = citySetPerPriority.get( oneLine.machineNetworkTypePriority);
            if( citySet==null){
                citySet = new HashSet<>();
                citySetPerPriority.put(oneLine.machineNetworkTypePriority, citySet );
            }
            citySet.add(oneLine.city);
        }

        Map<String, Map<Integer, Map<String, Integer>>> output_cityNumberOfMachine = new HashMap<>();
        for( Map.Entry<String, Map<Integer, Map<String, Set<String>>>> entry : existingCityOfMachine.entrySet() ){
            ...
            output_cityNumberOfMachine.put(quyu, output_cityNumberPerPriorityYear);
        }
        return output_cityNumberOfMachine;
    }
