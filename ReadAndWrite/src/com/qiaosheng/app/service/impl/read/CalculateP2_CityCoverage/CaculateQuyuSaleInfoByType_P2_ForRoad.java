...
public class CaculateQuyuSaleInfoByType_P2_ForRoad implements IBasicServiceInterface {

    private static final Logger log = LoggerFactory.getLogger(CaculateQuyuSaleInfoByType_P2_ForRoad.class);
    ...
    public void generateText(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        String fileName = BasicConfiguredValues.PAGE2And3_OUTPUT_DIRECTORY +  "公路车城市覆盖程度.txt";
        OutputToTxtUtil.writeTo( outputStringBuffer, fileName);
        log.info("Write the      公路车覆盖度 to file: " + fileName);
...
//        OutputToTxtUtil.writeTo(outputStringBuffer, "");
    ...
    private Map<String, Map<Integer, Map<String, Integer>>>  readByNetwork(List<JinpinOneLinePOJO> jinpinOneList) {
        ...
        Map<String, Map<Integer, Map<String, Set<String>>>> existingCityOfRoad = new HashMap<>();

//        第一遍是为了读取： 销量 跟城市列表 / city and year
        for (JinpinOneLinePOJO oneLine : jinpinOneList) {
            Map<Integer, Map<String, Set<String>>> citySetPerYearPriority = existingCityOfRoad.get(oneLine.quYu);
            if(citySetPerYearPriority==null){
                citySetPerYearPriority = new HashMap<>();
                existingCityOfRoad.put(oneLine.quYu, citySetPerYearPriority);
            }
            ...
            Set<String> citySet = citySetPerPriority.get( oneLine.roadNetworkTypePriority);
            if( citySet==null){
                citySet = new HashSet<>();
                citySetPerPriority.put(oneLine.roadNetworkTypePriority, citySet );
            }
            citySet.add(oneLine.city);
        }

        Map<String, Map<Integer, Map<String, Integer>>> output_cityNumberOfRoad = new HashMap<>();
        for( Map.Entry<String, Map<Integer, Map<String, Set<String>>>> entry : existingCityOfRoad.entrySet() ){
            ...
            output_cityNumberOfRoad.put(quyu, output_cityNumberPerPriorityYear);
        }
        return output_cityNumberOfRoad;
    }
